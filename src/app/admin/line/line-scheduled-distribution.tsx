            ...
            {messages.map((item) => (
              <article key={item.id} className="grid gap-3 rounded-md border border-slate-200 p-4 md:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)_auto] md:items-center">
                ...
                <div className="flex flex-wrap items-center gap-2 md:justify-end">
                  <span className={`rounded-md px-2.5 py-1 text-xs font-bold ${statusClasses[item.status]}`}>{item.status}</span>
                  {item.status === "予約中" ? <button type="button" onClick={() => setPendingCancel(item)} className="h-9 rounded-md border border-red-300 bg-white px-3 text-sm font-bold text-red-600 hover:bg-red-50">取消</button> : null}
                </div>
              </article>
            ))}
          </div>
