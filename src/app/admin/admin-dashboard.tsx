...
export function AdminDashboard() {
  ...
  const todayAvailability = availability[todayDateKey];
      ...
      <main className="mx-auto grid max-w-7xl gap-6 px-5 py-6 sm:px-6 lg:px-8">
        ...
        <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-col gap-2 border-b border-slate-200 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
            <div>
              <h2 className="text-base font-semibold">本日の予約状況</h2>
              <p className="mt-1 text-sm text-slate-500">
                {formatSelectedDate(todayDateKey)} の時間枠別の受付状況です。
              </p>
            </div>
            {todayAvailability?.holiday ? (
              <span className="w-fit rounded-full bg-slate-100 px-3 py-1 text-sm font-semibold text-slate-600 ring-1 ring-slate-200">
                休業日
              </span>
            ) : null}
          </div>
          <div className="grid gap-3 p-4 sm:grid-cols-2 sm:p-5 lg:grid-cols-7">
            {reservationTimeSlots.map((time) => {
              const slot = todayAvailability?.slots?.[time];
              ...
              return (
                <div
                  ...
                  className={[
                    ...
                    isStopped || todayAvailability?.holiday
                ...
                >
                  ...
                  {!isStopped && !todayAvailability?.holiday && !isFull ? (
                    <p className="mt-1 text-xs font-semibold text-blue-700">
                      残り {capacity - reservedCount}台
                    </p>
                  ) : null}
                </div>
              );
