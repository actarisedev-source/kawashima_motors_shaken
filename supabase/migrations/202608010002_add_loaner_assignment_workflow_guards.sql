...
begin
  ...
  if v_current.status not in ('scheduled', 'checked_out') then
    raise exception using
      errcode = 'P0001',
  ...
  if v_current.status = 'checked_out'
    and p_loaner_vehicle_id <> v_current.loaner_vehicle_id then
    raise exception using
      errcode = 'P0001',
      message = 'loaner_checked_out_vehicle_change_not_allowed';
  end if;
...
create or replace function public.checkout_loaner(
  p_assignment_id uuid
)
returns setof public.loaner_assignments
language plpgsql
...
begin
  ...
  if v_assignment.status <> 'scheduled' then
    raise exception using
      errcode = 'P0001',
      message = 'loaner_assignment_not_checkoutable';
  end if;

  select r.status, r.loaner_car_requested
  into v_reservation_status, v_loaner_car_requested
  from public.reservations r
  where r.id = v_assignment.reservation_id
  for update;
  ...
  -- actual_checked_out_at is intentionally not stored in Phase 2-4.
  -- scheduled_start_at remains the planned start and updated_at is not an actual checkout timestamp.
  update public.loaner_assignments
...
begin
  ...
  if not p_requested
    ...
    and v_assignment.status = 'checked_out' then
    raise exception using
      errcode = 'P0001',
  ...
  if not p_requested
    and v_assignment.id is not null
    and v_assignment.status = 'scheduled' then
    update public.loaner_assignments
    set status = 'cancelled',
...
begin
  ...
  if v_assignment.id is not null
    ...
    raise exception using
      errcode = 'P0001',
      message = 'loaner_checked_out_blocks_reservation_cancel';
  end if;
...
revoke all on function public.checkout_loaner(
  uuid
) from public, anon, authenticated;
...
grant execute on function public.checkout_loaner(
  uuid
) to service_role;
